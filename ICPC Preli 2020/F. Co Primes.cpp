...
int main() {
    ...
    Test {
        ...
        ll x = n;
        vector <ll> pf;
        for(ll i = 0 ; i < int(prime.size()) && prime[i] * prime[i] <= n ; i++) {
            if (n < ll(mxN)) {
                if (isp[n] == 0) break;
            }
            if(n % prime[i] == 0) {
        ...
        for (int i = 0; i < int(v.size()); i++) {
            ...
            if (int(v[i].size()) % 2 == 1) divisible += Range_Divisible_by_a(l , r , lc);
            else divisible -= Range_Divisible_by_a(l, r, lc);
        }
        ll sum = r - l + 1;
        ll ans = sum - divisible;
        cout << "Case " << tc << ": " << ans << "\n";
    }
    return 0;
}